...
var resizePizzas = function(size) { 
  ...
  // SV CHANGE If we output to console:  console.log("new width: " + newwidth); in a loop we will find that the size of pizzaContainer is same. So we can declare variables outside of the loop
...
// SV CHANGE assign pizzasDiv to variable outside the loop could not find much iumprovement
...
function updatePositions() {
  ...
  //SV CHANGE assign scrollTOp property to varibale to increase perfomance
  ...
  // SV CHANGE so I am trying use translateX according Paul Lewis recommendation http://csstriggers.com/ because it is only triggers composite 
  // howeever I still getting paint spikes in dev tools
  /* var winWidth= window.innerWidth;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(( scrollTop/ 1250) + (i % 2));
    //items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // SV CHANGE decreased the number of generated pizzas to 35 to improve perfomance 
  for (var i = 0; i < 35; i++) {
